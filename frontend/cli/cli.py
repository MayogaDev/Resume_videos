...
try:
    ...
    print(f"❌ Error: No se pueden importar los módulos del backend: {e}")
    sys.exit(1)
...
def procesar_video_interactivo(
    ...
    with Progress(
        ...
        try:
            ...
            transcriber = VideoTranscriber(model_size=whisper_model, language="es", verbose=False)
            ...
            result = transcriber.transcribe_video(video_path, save_transcript=False, cleanup_audio=True)
...
@click.option('--modelo', '-m', default='google/mt5-small',
              help='Ruta al modelo mT5 (HuggingFace o local)')
@click.option('--whisper', '-w', default='tiny',
